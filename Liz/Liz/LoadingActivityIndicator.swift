...
class LoadingActivityIndicator : UIView {
	...
	private func createLayers() {
		let backgroundView = UIView(frame:CGRectMake(0, 0, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame)))
		backgroundView.layer.borderColor = UIColor.blackColor().CGColor
		backgroundView.backgroundColor = UIColor(white:0.1, alpha:1.0)
		
		let dot = UIBezierPath(arcCenter: CGRectGetCenter(backgroundView.frame), radius: self.circleLineWidth, startAngle:-0.5 * CGFloat(M_PI), endAngle:1.5 * CGFloat(M_PI), clockwise:true)
		...
		5.times { [unowned self] (index: Int) in
			...
			let path = UIBezierPath(arcCenter: self.CGRectGetCenter(backgroundView.frame), radius: radius, startAngle:-0.5 * CGFloat(M_PI), endAngle:1.5 * CGFloat(M_PI), clockwise:true)
